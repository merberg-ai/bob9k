...
function renderGlobalStatus(rawData){
  ...
  window.bob9kState.lastStatus = data;
  document.dispatchEvent(new CustomEvent('bob9k:status', {detail: data}));
}

let driveHoldTimer = null;
window.bob9kState = window.bob9kState || { lastStatus: null, refreshTimer: null, isRefreshing: false };
...
function bindHoldButton(actionName, handler){
  ...
  const stop = async (event) => {
    ...
    stopDriveHold();
    await window.bob9kApi.motorStop();
    await refreshStatus();
  };
...
async function refreshStatus(force=false){
  if(window.bob9kState.isRefreshing && !force){
    return window.bob9kState.lastStatus;
  }
  window.bob9kState.isRefreshing = true;
  try{
    const data = await window.bob9kApi.getStatus();
    ...
    window.bob9kState.isRefreshing = false;
  }
}

function startStatusPolling(intervalMs=2000){
  if(window.bob9kState.refreshTimer){
    clearInterval(window.bob9kState.refreshTimer);
  }
  window.bob9kState.refreshTimer = setInterval(()=>{
    refreshStatus().catch(()=>{});
  }, intervalMs);
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(estopButton){
    estopButton.addEventListener('click', async()=>{
      await invokeAndRefresh(()=>window.bob9kApi.motorEstop(), {
