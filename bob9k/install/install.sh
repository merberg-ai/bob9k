...
# bob9k installer
# Installs bob9k into the current user's home directory and sets up systemd boot service.

APP_NAME="bob9k"
SERVICE_NAME="bob9k.service"
...
if [[ ! -f "${PROJECT_DIR}/app.py" ]]; then
    ...
    err "Run this script from the bob9k project tree."
    exit 1
fi
...
if [[ -f /etc/rc.local ]] && grep -qiE 'adeept|picar|webServer|bob9k|python.*app.py' /etc/rc.local; then
    warn "/etc/rc.local appears to contain robot/autostart entries."
    CONFLICTS_FOUND=1
fi

if pgrep -af 'webServer|Adeept|bob9k|app.py' >/dev/null 2>&1; then
    warn "Found existing Python robot-ish processes:"
    pgrep -af 'webServer|Adeept|bob9k|app.py' || true
    CONFLICTS_FOUND=1
fi
...
cat > "${TMP_SERVICE}" <<EOF
[Unit]
Description=bob9k robot control service
...
# ---------- restart service ----------
step "Starting bob9k service"

sudo systemctl restart "${SERVICE_NAME}"
sleep 2

if systemctl is-active --quiet "${SERVICE_NAME}"; then
    ok "bob9k service is running"
else
    warn "bob9k service did not come up cleanly"
    echo
